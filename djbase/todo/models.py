...
class Todo(models.Model):
    title = models.CharField("曲名",max_length=30)
    artist = models.CharField("歌手名", null=True, max_length=20)
    description = models.TextField("コメント", blank=True)
    datetime = models.DateTimeField("登録日", auto_now=True)
    hiki_TYPES = (
        ('-指弾き-', '指弾き'),
        ('-指弾き・一部スラップ-', '指弾き・一部スラップ'),
        ('-スラップ-', 'スラップ')
    )
    hiki_type = models.CharField("弾き方",max_length=50, choices=hiki_TYPES, default="指弾き")
    share_url = models.URLField("埋め込みURL", blank=True)
